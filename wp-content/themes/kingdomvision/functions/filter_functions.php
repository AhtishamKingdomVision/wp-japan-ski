...
define('KV_BS_authToken', '10530|1AOM2D6filRSzAyD7GWpV9Vy8za5sBNK32r6rE9G45ce4a71');
...
function get_accommodation_base_areas( $resort = null ) {
    ...
    // Slugs may be 'niseko' or 'niseko-accommodation'.
    ...
    // Fetch all child categories for the resort.
    ...
    try {
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                // Show Expert Recommendation CTA after the first 3 cards on page 1
                if ($count === 3 && (isset($args['paged']) ? (int)$args['paged'] : 1) === 1) {
                    echo kv_get_expert_recommendation_cta();
                }

                // Show Stronger Recommendation CTA at the end of page 2
                if ($count === 6 && (isset($args['paged']) ? (int)$args['paged'] : 1) === 2) {
...
function kv_roomboss_available_ids_cached(
    ...
    try {
        ...
        cf_log( $bs_args, 'acc_bs_args', 'txt', true, true );
...
function hz_get_local_property_ids_and_args(array $input): array {
    ...
    try {
        ...
        if (!empty($input['property_id'])) {
            $property = sanitize_text_field($input['property_id']);
            if (!empty($property)) {
                $booking_sys_args['propertyIds'] = [$property];
            }
        }
...
function niseko_build_search_query_args(array $input, array $overrides = []): array
{
    ...
    cf_log( 'args', 'cf_args' );
    cf_log( $args, 'cf_args' );
    cf_log( 'overrides', 'cf_args' );
    cf_log( $overrides, 'cf_args' );
    cf_log( 'txt-q', 'cf_args' );
    cf_log( array_replace_recursive($args, $overrides), 'cf_args' );
...
function niseko_search_roomboss_wp(array $availableHotelIds, array $roombossData = []) {
    try {
        ...
        // Validate roombossData is array or set to empty if invalid
        if (!is_array($roombossData)) {
            $roombossData = [];
        }

        // ✅ STEP 2: Get room count from available hotels
        $room_count = get_room_count_from_roomboss_hotels($availableHotelIds);
        $room_count = intval($room_count);

        // ✅ STEP 3: Build WP_Query arguments with available hotel IDs filter
        $args = niseko_build_search_query_args($_POST, [
            'meta_query' => [
                'relation' => 'AND',
                [
        ...
        // ✅ STEP 5: Build HTML output from matching posts
        ob_start();
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                // Validate property ID exists in roomboss data
                ...
                // Render accommodation card template
        ...
        // ✅ STEP 6: Prepare and send JSON response
...
function niseko_load_room_details()
{
    try {
        ...
        $hotel_id = isset($_POST['hotel_id']) ? sanitize_text_field($_POST['hotel_id']) : 0;
        ...
        // ✅ STEP 2: Type-cast hotel_id appropriately
...
function kv_get_expert_recommendation_cta() {
    ...
    $html = '<div class="expert-recommendation-cta" style="grid-column: 1 / -1; width: 100%; margin: -69px 0px 3px 0px; color: #fff; overflow: hidden; display: flex; align-items: center; position: relative; font-family: inherit;">';
    
        // Left Content Area
        $html .= '<div style="flex: 1.5; padding: 30px 15px; z-index: 2; position: relative; display: flex; align-items: center; justify-content: space-between; gap: 30px;">';
            if(get_field('exp_acc_content', 'options')) {
                $html .= '<p>'.get_field('exp_acc_content', 'options').'</p>';
            }
        ...
        $html .= '</div>';// End of Left Content Area
...
function kv_get_stronger_recommendation_cta() {
    ...
    $sec_cont = $sec_cont ? $sec_cont : 'Let us recommend the best available options
for your dates.';
    ...
    $html = '<div class="strong-recommendation-cta stronger-message" style="max-width: 740px; width: 100%; margin: 0 auto 0px; padding: 24px; background: #00274E; border-radius: 16px; text-align: center; margin-bottom: 60px;">';
    
        $html .= '<div class="acco cta" style="flex: 1.5; padding: 30px 15px; z-index: 2; position: relative; align-items: center; justify-content: space-between; gap: 30px;">';
                $html .= '<h3 style="margin: 0 0 5px 0; font-size: 20px; color: #fff; line-height: 1.3;">'.$sec_cont.'</h3>';
            $html .= '<a style="margin-top: 20px;" class="btn" href="'.$cta_link.'" class="expert-cta-button btn">Get Expert Recommendations</a>';
...
function kv_get_resort_areas_ajax() {
    ...
    if (empty($resort)) {
        wp_send_json_error(['message' => 'Resort parameter is required']);
    }

    // get_accommodation_base_areas helper is defined in accommodation-config.php
